...
class TestHomeWork10(unittest.TestCase):
    ...
    def test_020(self):
        """
        20. Класс Birthday в коді оголошено
        """
        assert hasattr(main, "Birthday"), """20. Класс Birthday в коді не оголошено!"""

    def test_021(self):
        """
        21. Класс Birthday наслідується від класу Field в коді
        """
        if hasattr(main, "Birthday"):
            assert (
                main.Birthday.__base__ == main.Field
            ), """21. Класс Birthday не наслідується від класу Field в коді"""
        else:
            raise AssertionError(
                """21. Класс Birthday не наслідується від класу Field в коді"""
            )
    ...
    def test_022(self):
        """
        22. Класс AddressBook має метод iterator
        """
        msg = """22. Класс AddressBook не має метод iterator!"""
        if hasattr(main, "AddressBook"):
            assert "iterator" in dir(main.AddressBook), msg
        else:
            raise AssertionError(msg)

    def test_023(self):
        """
        23. Класс Record має метод days_to_birthday
        """
        msg = """23. Класс Record не має метод days_to_birthday!"""
        if hasattr(main, "Record"):
            assert "days_to_birthday" in dir(main.Record), msg
        else:
            raise AssertionError(msg)

    def test_024(self):
        """
        24. Клас Name зберігає валідне ім'я "John" в атрибуті value
        """
        msg = """24. Клас Name не зберігає валідне ім'я "John" в атрибуті value"""
        if hasattr(main, "Name"):
            name = main.Name("John")
            self.assertTrue(hasattr(name, "value"), msg)
            self.assertEqual(name.value, "John", msg)
        else:
            self.fail(msg)

    def test_025(self):
        """
        25. Клас Name видає виняток при передачі невалідного імені з цифрами
        """
        msg = """25. Клас Name не видає виняток при передачі невалідного імені з цифрами"""
        if hasattr(main, "Name"):
    ...
    def test_026(self):
        """
        26. Клас Birthday зберігає валідну дату народження "01-01-2000" в атрибуті value
        """
        msg = """26. Клас Birthday не зберігає валідну дату народження "01-01-2000" в атрибуті value"""
        if hasattr(main, "Birthday"):
            birthday = main.Birthday("01-01-2000")
            self.assertTrue(hasattr(birthday, "value"), msg)
            self.assertEqual(birthday.value, "01-01-2000", msg)
        else:
            self.fail(msg)

    def test_027(self):
        """
        27. Клас Birthday видає виняток при передачі невалідної дати народження "30-02-2000"
        """
        msg = """27. Клас Birthday не видає виняток при передачі невалідної дати народження "30-02-2000" """
        if hasattr(main, "Birthday"):
            with self.assertRaises(ValueError):
                main.Birthday("30-02-2000")
        else:
            self.fail(msg)
